...
if (!empty($_SESSION['admin'])) {
    ...
    if (!empty($_GET['barang'])) {
        ...
        $data[] = $tgl;
        $data[] = $id;
        $sql = 'UPDATE barang id_kategori=?, nama_barang=?, merk=?, harga_beli=?, harga_jual=?, satuan_barang=? stok=?, tgl_update=? WHERE id_barang=?';
        $row = $config->prepare($sql);
        $row->execute($data);
    ...
    if (!empty($_GET['profil'])) {
        ...
        $row = $config->prepare($sql);
        $row->execute($data);
        echo '<script>window.location="../../index.php?page=user&succes=edit-data"</script>';
    }
    if (!empty($_GET['pass'])) {
        $id = htmlentities($_POST['id']);
    ...
    if (!empty($_GET['jual'])) {
        $id = htmlentities($_POST['id']);
        $id_barang = htmlentities($_POST['id-barang']);
    ...
    if (!empty($_GET['cari_barang'])) {
        ...
        if ($cari == '') {
            ...
            $sql = "select barang.*, kategori.id_kategori, kategori.nama_kategori from barang inner join kategori on barang.id_kategori = kategori.id_kategori
